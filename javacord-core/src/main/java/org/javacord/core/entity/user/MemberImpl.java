...
public final class MemberImpl implements Member {
    ...
    /**
     * Creates a new immutable member instance from a Member object.
     *
     * @param api The api instance.
     * @param server The server of the member.
     * @param member The member object to build this MemberImpl from.
     */
    public MemberImpl(DiscordApiImpl api, ServerImpl server, MemberImpl member) {
        // This constructor essentially has to exist because MemberImpl is immutable
        // but we may need to create a new one on a VOICE_STATE_UPDATE without a Member data packet.
        this(api, server, member.user, member.nickname, member.roleIds, member.joinedAt,
                member.serverBoostingSince, member.muted, member.deafened, member.selfMuted, member.selfDeafened);
    }
